...
<div class = "home" align = right>
			<input type="button" value="清空購物車" onclick="deletea();" style="width:180px;height:40px;font-size:20px;"/>
...
<?php
	...
	//mysqli_close($link);
		...
		$sql="SELECT * FROM meal WHERE meal_Series='ser01' ";
...
<script language="javascript" >
	...
	function setpage(mealname,price){
    	...
    	storageArray.push({ mealname: mealname , price: price  , customize : test ,is_set: true });
    	localStorage.setItem(`mealname`, JSON.stringify(storageArray));
    	mealname = '';
		price = '';
		test = '';
	...
	function single(mealname,price){
		...
		event.preventDefault();    	
    	storageArray.push({ mealname: mealname , price: price  , customize : test , is_set: false});
    	localStorage.setItem(`mealname`, JSON.stringify(storageArray));
    	mealname = '';
		price = '';
		test = '';
